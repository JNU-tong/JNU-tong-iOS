...
class CityBusDetailViewController: UIViewController {
    ...
    @objc private func setLine(_ notification: Notification) {
        // swiftLint fix
        var cityBusLine = [String]()

        if let lineData = notification.userInfo!["lineData"] as? [String] {
            cityBusLine = lineData
        }
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "setBusLineInfo"),
                                        object: nil,
                                        userInfo: ["lineData": cityBusLine, "busInfo": busInfo!])
    }

    @objc private func setTime(_ notification: Notification) {
        var cityBusTime = [String]()
        var cityBusRemainTime = [Int]()

        if let departData = notification.userInfo!["departData"] as? [String] {
            cityBusTime = departData
        }
        if let remainData = notification.userInfo!["remainData"] as? [Int] {
            cityBusRemainTime = remainData
        }
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "setBusTimeInfo"),
                                        object: nil,
                                        userInfo: ["departTime": cityBusTime, "remainTime": cityBusRemainTime, "busInfo": busInfo!])
    }
