...
class MainViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.navigationController?.navigationBar.barTintColor = UIColor(red: CGFloat(0.0 / 255.0),
        ...
        if segue.identifier == "cityBusSegue" {
            if let indexPath = cityBusTable.indexPathForSelectedRow {
                if indexPath.section == 0,
                    let cityBusDetailViewController = segue.destination as? CityBusDetailViewController {
                    let busInfo = favoriteBusList[indexPath.row]
                    let cityBusView = cityBusDetailViewController
            ...
            let shuttleBusDetailViewController = segue.destination as? ShuttleBusDetailViewController {
            let shuttleBusView = shuttleBusDetailViewController
...
extension MainViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            ...
            headerView.layer.backgroundColor = UIColor.init(red: CGFloat(238.0 / 255.0),
                                                            green: CGFloat(238.0 / 255.0),
                                                            blue: CGFloat(238.0 / 255.0), alpha: 1).cgColor
            return headerView
        } else if section == 1 {
            let headerText = UILabel()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section==1, let cell = tableView.dequeueReusableCell(withIdentifier: "cityBusCell", for: indexPath) as? CityBusCell {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "favoriteBusCell", for: indexPath) as? FavoriteBusCell {
